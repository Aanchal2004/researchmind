...
# Protocol-compatible type for both SynthesisService and LLMSynthesisService.
from typing import Protocol


class SynthesisProtocol(Protocol):
    async def synthesize(self, *, query: str, papers: Sequence[SearchResultItem]) -> SearchSynthesis:
        ...
...
class SearchService:
    def __init__(
        ...
        default_limit: int,
        max_limit: int,
        synthesis_service: SynthesisProtocol | None = None,
        unpaywall_enricher: UnpaywallEnricher | None = None,
    ) -> None:
        self.providers = list(providers)
        self.default_limit = default_limit
        self.max_limit = max_limit
        self.result_merger = SearchResultMerger()
        self.synthesis_service: SynthesisProtocol = synthesis_service or SynthesisService(
            enabled=True,
            max_papers=5,
            max_abstract_chars=1200,
        )
        self.unpaywall_enricher = unpaywall_enricher
    ...
    async def search(
        ...
        """Run retrieval + ranking.  Synthesis is NOT awaited here.

        The caller (route handler) fires synthesis as a BackgroundTask so
        the response returns immediately with synthesis.status="pending".
        ``query_id`` is generated by the route and passed in so the
        background task can store its result under the same key.
        """
        _query_id = query_id or str(uuid4())
        ...
        # Best-effort Unpaywall enrichment — attach open-access PDF URLs by DOI.
        if self.unpaywall_enricher is not None:
            ranked_results = await self.unpaywall_enricher.enrich(ranked_results)
        ...
        # Return "pending" synthesis immediately — the route fires the real
        # synthesis as a BackgroundTask.
        pending_synthesis = (
            SearchSynthesis(status="pending", summary="", highlights=[], sources=[])
            if ranked_results
